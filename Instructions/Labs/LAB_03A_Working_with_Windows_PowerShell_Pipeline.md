...
> **注:** Windows PowerShell 5.1 では、このタスクに `Get-EventLog` を使用します。 PowerShell 7 では代わりに `Get-WinEvent` を使用します。それによって、`EventID` ではなく `Id` プロパティが、`TimeWritten` ではなく `TimeCreated` が使用されます。
