...
## 演習 1: PowerShell バインドを使用したデータの選択、並べ替え、表示
...
1. コンソールで、次のコマンドを入力して、コマンドレットで受け入れられるパイプライン バインドの種類を確認します。

   ```powershell
   Get-Help Get-Date -Parameter * | Where-Object { $_.PipelineInput -match "true" } 
      ```

   > **注:** **受け入れパイプラインの入力**と**パラメーター セット名**の値に注意します。
...
1. コンソールで、次のコマンドを入力して Enter キーを押します。  
   ...
   > **注:** **(|)** を使用すると、1 つのコマンドレットの出力を別のコマンドレットに入力として渡すことができます。
   **DayOfYear** プロパティに注意します。
...
> **注:** 以下のコマンドは Windows PowerShell 5.1 でのみ使用可能な `Get-EventLog` を使用します。 PowerShell 7 では代わりに `Get-WinEvent -LogName Security | Where-Object Id -eq 4624` を使用してください。 `TimeWritten` を`TimeCreated`、`EventID` を `Id` に置き換えます。
