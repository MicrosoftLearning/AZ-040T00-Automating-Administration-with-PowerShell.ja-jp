...
1. テストとして、**Get-CimInstance** を使用して、ローカル ハード ドライブ (ドライブの種類が **3** のドライブだけを含むようにフィルターを適用した **Win32_LogicalDisk** クラス) のリストを表示します。
1. **LON-DC1** と **LON-CL1** の両方に対して、リモート処理を使用して **Get-CimInstance** コマンドをもう一度実行します。 **Get-CimInstance** コマンドに **–ComputerName** パラメーターを追加しないでください。
